...
export class SampleServiceService {
    ...
    if (file.type == 'text/csv') {
      this.papa.parse(file, {
        complete: function (results) {
          ...
          const answers = [];
          for (const line of JSON.parse(localStorage.getItem('ImportedCSV'))) {
            answers.push(line[1]);
          }
          let count = 1;
          const neueListe = {};
  ...
  stringToDate(datestring: string) {
    ...
    const date = new Date(parts[2].concat('-', parts[1], '-', parts[0]));
    return date;
  }
  ...
  getSubjects(): Subject[] {
    ...
    for (const k in this.globalItems) {
      ...
      const due = new Date(this.globalItems[k].due);
      if (due <= today) {
        const index = subjectList.findIndex(x => x.subject == this.globalItems[k].subject);
  ...
  getItemsForSubject(subject: string): QuizItem[] {
    ...
    for (const k in this.globalItems) {
      if (this.globalItems[k].subject == subject) {
        Items.push({
          'ID': parseInt(k),
          'question': this.globalItems[k].question,
          'answer': this.globalItems[k].answer,
          'subject': this.globalItems[k].subject,
          'phase': this.globalItems[k].phase,
          'due': this.globalItems[k].due,
          'swappedTo': this.globalItems[k].swappedTo,
        });
      }
    }
    return Items;
  ...
  updateItem(quizItem: QuizItem) {
    ...
    this.globalItems[quizItem.ID] = {
      'question': quizItem.question,
      'answer': quizItem.answer,
      'subject': quizItem.subject,
      'phase': quizItem.phase,
      'due': quizItem.due,
      'swappedTo': quizItem.swappedTo,
    };
    this.saveProgress();
  }
  ...
  addItem(Item) {
    ...
    if (Item.swapped == false) {
      const currentMax: number = this.getMax(this.globalItems);
      this.globalItems[(currentMax + 1)] = {
      ...
      this.globalItems[(currentMax + 1)] = {
        ...
        'phase': 1,
        'due': today.toISOString().slice(0, 10),
        'swappedTo': currentMax + 2
      };
      this.globalItems[(currentMax + 2)] = {
        'question': Item.answer,
        'answer': Item.question,
        'subject': Item.subject,
        'phase': 1,
        'due': today.toISOString().slice(0, 10),
        'swappedTo': currentMax + 1
      };
    }
    this.saveProgress();
  }
  ...
  getRandQuizItem(subject: string): QuizItem {
    ...
    const random: number = listOfDue[this.getRandom(listOfDue.length)];
    return {
      'ID': random,
      'question': this.globalItems[random].question,
      'answer': this.globalItems[random].answer,
      'subject': this.globalItems[random].subject,
      'phase': this.globalItems[random].phase,
      'due': this.globalItems[random].due,
      'swappedTo': this.globalItems[random].swappedTo,
    };
  }
  ...
  updateAfterQuiz(quizItem: QuizItem, answerCorrect: boolean): QuizItem {
    if (answerCorrect == true) {
      if (quizItem.phase == 6) {
        ...
        const newDue = new Date(this.updateDueDate(quizItem.phase));
  ...
  getItem(id: number): QuizItem {
    return {
      'ID': id,
      'question': this.globalItems[id].question,
      'answer': this.globalItems[id].answer,
      'subject': this.globalItems[id].subject,
      'phase': this.globalItems[id].phase,
      'due': this.globalItems[id].due,
      'swappedTo': this.globalItems[id].swappedTo,
    };
  }
